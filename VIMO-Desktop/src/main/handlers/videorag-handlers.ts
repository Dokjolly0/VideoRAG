...
async function scanForVideoRAGService(
  startPort?: number,
  endPort?: number,
): Promise<number | null> {
    ...
    try {
      ...
      if (isDev) {
        ...
        const executableName =
          process.platform === "win32" ? "videorag_api.exe" : "videorag_api";
        ...
        if (!fs.existsSync(executablePath)) {
          console.error(
            `❌ Packaged executable not found at: ${executablePath}`,
          );
          safeReject(
            new Error(`VideoRAG executable not found at: ${executablePath}`),
          );
          return;
        }
        ...
        try {
          console.log(
            `🚀 Starting VideoRAG service with packaged executable: ${executablePath}`,
          );
          ...
          safeReject(
            new Error(`Failed to start packaged executable: ${error}`),
          );
          return;
        }
        ...
        if (!pythonProcess) {
          safeReject(
            new Error(
              "Failed to start packaged executable. Please ensure the executable was built correctly.",
            ),
          );
          return;
        }
      ...
      setTimeout(async () => {
        if (!resolved) {
          console.log(
            "⏳ Initial timeout reached, starting continuous service detection...",
          );
          ...
          const continuousScan = async () => {
            scanAttempts++;
            console.log(
              `🔍 Scanning attempt ${scanAttempts}/${maxScanAttempts}...`,
            );

            const foundPort = await scanForVideoRAGService(
              PORT_RANGE_START,
              PORT_RANGE_END,
            );
            if (foundPort) {
              console.log(
                `✅ Found service on port ${foundPort} after ${scanAttempts} attempts!`,
              );
              ...
              initializeVideoRAGConfig()
                ...
                .catch(() => {
                  console.error(
                    "Configuration initialization failed, but service is running",
                  );
            ...
            if (scanAttempts < maxScanAttempts) {
              console.log(
                `❌ Service not found, retrying in ${scanInterval / 1000} seconds... (${scanAttempts}/${maxScanAttempts})`,
              );
              setTimeout(continuousScan, scanInterval);
            } else {
              console.error(
                `❌ Failed to locate VideoRAG service after ${maxScanAttempts} scan attempts`,
              );
              safeReject(
                new Error(
                  `Failed to locate VideoRAG service after ${maxScanAttempts} scan attempts over ${(maxScanAttempts * scanInterval) / 1000} seconds`,
                ),
              );
            }
          };
...
async function callVideoRAGAPI(
  endpoint: string,
  ...
  if (endpoint.includes("/upload")) {
    ...
    endpoint.includes("/imagebind/load") ||
    endpoint.includes("/imagebind/release")
  ) {
  ...
  try {
    ...
    if (error.code === "ECONNABORTED") {
      throw new Error(
        `Request timeout after ${timeout}ms. The operation might need more time to complete.`,
      );
    }
    throw new Error(
...
async function initializeVideoRAGConfig(): Promise<void> {
  try {
    ...
    console.log("🔧 VideoRAG configuration validation:", {
      ali_dashscope_api_key: videoragConfig.ali_dashscope_api_key
        ? "✅ SET"
        : "❌ MISSING",
      ...
      image_bind_model_path: videoragConfig.image_bind_model_path
        ? "✅ SET"
        : "❌ MISSING",
      base_storage_path: videoragConfig.base_storage_path
        ? "✅ SET"
        : "❌ MISSING",
    ...
    if (
      !videoragConfig.ali_dashscope_api_key ||
      videoragConfig.ali_dashscope_api_key.trim() === ""
    ) {
      missingFields.push("Ali Dashscope API Key (dashscopeApiKey)");
    }

    if (
      !videoragConfig.openai_api_key ||
      videoragConfig.openai_api_key.trim() === ""
    ) {
      missingFields.push("OpenAI API Key (openaiApiKey)");
    }

    if (
      !videoragConfig.base_storage_path ||
      videoragConfig.base_storage_path.trim() === ""
    ) {
      missingFields.push("Base Storage Path (storeDirectory)");
    }

    if (
      !videoragConfig.image_bind_model_path ||
      videoragConfig.image_bind_model_path.trim() === ""
    ) {
      missingFields.push(
        "ImageBind Model Path (storeDirectory + imagebind_huge/imagebind_huge.pth)",
      );
    }
    ...
    if (missingFields.length > 0) {
      ...
      console.log(
        "🛑 Stopping VideoRAG service due to configuration errors...",
      );
      stopVideoRAGService();
    ...
    try {
      ...
      throw new Error(
        `ImageBind model file not found: ${videoragConfig.image_bind_model_path}`,
      );
    }
    ...
    try {
      ...
      try {
        ...
        console.log(
          "✅ Storage directory verified:",
          videoragConfig.base_storage_path,
        );
      } catch (error) {
        // Directory not found, attempt to create
        console.log(
          "📁 Storage directory not found, attempting to create:",
          videoragConfig.base_storage_path,
        );
      ...
      // Stop VideoRAG service
      console.log(
        "🛑 Stopping VideoRAG service due to storage directory error...",
      );
      stopVideoRAGService();

      throw new Error(
        `Storage directory error: ${videoragConfig.base_storage_path}`,
      );
    }
    ...
    if (result.success) {
      ...
      // Stop VideoRAG service
      console.log(
        "🛑 Stopping VideoRAG service due to API initialization failure...",
      );
      stopVideoRAGService();
...
async function loadSettingsFromFile(): Promise<{
  success: boolean;
  settings?: any;
  error?: string;
}> {
...
export function setupVideoRAGHandlers() {
  ...
  ipcMain.handle("videorag:service-status", async () => {
    try {
      ...
      return {
        success: true,
        isRunning,
        pythonProcess: isRunning ? "running" : "stopped",
      };
    } catch (error: any) {
      return { success: false, error: error.message };
    }
  ...
  ipcMain.handle(
    ...
    async (_, chatId: string, type?: string) => {
      try {
        const url = type
          ? `/sessions/${chatId}/status?type=${type}`
          : `/sessions/${chatId}/status`;
  ...
  ipcMain.handle("videorag:get-localStorage-config", async () => {
    try {
      ...
      return {
        success: true,
        message: "This handler should be called from renderer",
      };
    } catch (error: any) {
      return { success: false, error: error.message };
    }
...
async function attemptHealthCheck(port: number): Promise<boolean> {
  try {
    ...
    if (error.code === "ECONNREFUSED") {
      ...
      console.log(
        `🔍 Port ${port} connection reset, service might be starting`,
      );
    } else {
