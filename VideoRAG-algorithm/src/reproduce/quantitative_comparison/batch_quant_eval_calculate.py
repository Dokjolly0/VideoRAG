...
evaluate_model = ["llamavid", "videoagent", "notebooklm", "videorag"]
        ...
        for i in range(len(querys)):
            for _model in evaluate_model:
                ...
                evaluation_result = results[
                    f"{category}++query{query_id}++base++answers-{baseline_model}++evaluate++answers-{_model}"
                ]
                ...
                for _metric in metrics:
                    ...
                    domain_score[category_domain_dict[category_id]][_model][
                        _metric
                    ].append(_metric_score)
...
with open(f"batch_requests/{base_dir}/{base_dir}.txt", "a") as f:
    ...
    for _model in evaluate_model:
        ...
        for _domain in domain_list:
            ...
            for _metric in metrics:
                print(
                    f"{np.array(domain_score[_domain][_model][_metric]).mean():.2f}",
                    _metric,
                )
                f.write(
                    f"{np.array(domain_score[_domain][_model][_metric]).mean():.2f} {_metric}\n"
                )
            print("----")
            f.write("----\n")
        print("All")
        ...
        for _metric in metrics:
            ...
            f.write(
                f"{np.array(overall_score[_model][_metric]).mean():.2f} {_metric}\n"
            )
        print("====" * 8)
        f.write("====" * 8 + "\n")
