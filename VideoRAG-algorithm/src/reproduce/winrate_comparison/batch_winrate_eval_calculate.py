...
for category_id in tqdm(all_data):
    ...
    for _file in result_file:
        ...
        for i in range(len(querys)):
            ...
            ori_result = results[
                f"{category}++query{query_id}++answers-{model_a}++answers-{model_b}++ori"
            ]
            rev_result = results[
                f"{category}++query{query_id}++answers-{model_b}++answers-{model_a}++rev"
            ]
            ...
            for _metric in metrics:
                ...
                domain_win_count[category_domain_dict[category_id]][_metric][
                    winner
                ] += 1
...
with open(f"batch_requests/{base_dir}/{base_dir}.txt", "a") as f:
    ...
    for domain in domain_list:
        ...
        for _metric in metrics:
            total_count = (
                domain_win_count[domain][_metric]["a"]
                + domain_win_count[domain][_metric]["b"]
            )
            win_a_percentage = (
                domain_win_count[domain][_metric]["a"] / total_count
            ) * 100
            win_b_percentage = (
                domain_win_count[domain][_metric]["b"] / total_count
            ) * 100
            print(
                f"{win_a_percentage:.2f}% : {win_b_percentage:.2f}%",
                domain_win_count[domain][_metric],
                _metric,
            )
            f.write(
                f"{win_a_percentage:.2f}% : {win_b_percentage:.2f}% {domain_win_count[domain][_metric]} {_metric} \n"
            )
        print("----" * 8)
        f.write("----" * 8 + "\n")
    ...
    for _metric in metrics:
        ...
        print(
            f"{win_a_percentage:.2f}% : {win_b_percentage:.2f}%",
            overall_win_count[_metric],
            _metric,
        )
        f.write(
            f"{win_a_percentage:.2f}% : {win_b_percentage:.2f}% {overall_win_count[_metric]} {_metric} \n"
        )
    f.write("====" * 8 + "\n\n")
