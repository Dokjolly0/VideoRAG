...
if __name__ == "__main__":
    ...
    for _id in questions:
        ...
        for _evaluation_answer_dir in evaluation_answer_dir:
            ...
            for i in range(len(questions[_id][0]["questions"])):
                ...
                # baseline answer
                with open(
                    os.path.join(baseline_work_dir, f"answer_{query_id}.md"), "r"
                ) as f:
                    baseline_answer = f.read()
                # evaluation answer
                with open(
                    os.path.join(evaluation_work_dir, f"answer_{query_id}.md"), "r"
                ) as f:
    ...
    for k in range(run_time):
        ...
        batch = client.batches.create(
            ...
            metadata={
                "description": f"runtime{k} - a very nice and successful eval job: {request_json_file_path}"
            },
        )
