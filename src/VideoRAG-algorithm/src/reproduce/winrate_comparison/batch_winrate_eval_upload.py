    ...
    for _id in questions:
        ...
        for _com_answer_dir in com_answer_dir:
            ...
            for i in range(len(questions[_id][0]["questions"])):
                ...
                # our answer
                with open(
                    os.path.join(our_work_dir, f"answer_{query_id}.md"), "r"
                ) as f:
                    our_answer = f.read()
                # com answer
                with open(
                    os.path.join(com_work_dir, f"answer_{query_id}.md"), "r"
                ) as f:
                    com_answer = f.read()
                ori_prompt = prompt.format(
                    query=query, answer1=our_answer, answer2=com_answer
                )
                rev_prompt = prompt.format(
                    query=query, answer1=com_answer, answer2=our_answer
                )
