    ...
    for i in range(len(data)):
        ...
        try:
            json_data = json.loads(
                dp["response"]["body"]["choices"][0]["message"]["content"]
            )
            check_response_valid(json_data)
            parse_results[custom_id] = json_data
        except Exception as e:
            print(
                f"{_file} ({i}/{len(data)}) Find error when parsing {custom_id} ({e}), re-request OpenAI"
            )
            while True:
                try:
                    ...
                    json_data = json.loads(response.choices[0].message.content or "")
                    check_response_valid(json_data)
                    parse_results[custom_id] = json_data
