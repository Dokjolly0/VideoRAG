...
class LLMConfig:
    # To be set
    embedding_func_raw: Callable[..., Awaitable[np.ndarray]]
    ...
    best_model_func_raw: Callable[..., Awaitable[str]]
    best_model_name: str
    best_model_max_token_size: int
    best_model_max_async: int

    cheap_model_func_raw: Callable[..., Awaitable[str]]
    ...
    best_model_func: Optional[Callable[..., Awaitable[str]]] = None
    cheap_model_func: Optional[Callable[..., Awaitable[str]]] = None
    ...
    def __post_init__(self):
        ...
        self.best_model_func = lambda prompt, *args, **kwargs: (
            self.best_model_func_raw(self.best_model_name, prompt, *args, **kwargs)
        )

        self.cheap_model_func = lambda prompt, *args, **kwargs: (
            self.cheap_model_func_raw(self.cheap_model_name, prompt, *args, **kwargs)
        )
...
async def ollama_embedding(model_name: str, texts: list[str]) -> np.ndarray:
    ...
    response = await ollama_client.embed(model=model_name, input=texts)
